...
export function TodoList(){
  ...
  return (
    <div className='todo-wrapper'>
      <h1 className='name-wrapper'>Mes tâches :</h1>
...
const Form = ({addTodo}) => {
  ...
  const onSubmit = (event) => {
    ...
    const todoText = event.currentTarget.elements.todoText.value;
    console.log(todoText);
    addTodo(todoText);
  }
  return (
    <form className='form-todo-wrapper' onSubmit={onSubmit}>
      <input className='input-form' type="text" placeholder="Ajouter une tâche"/>
...
const Button = ({children,...props}) => {
  return <button className="btn-add-task" {...props}>{children}</button>
}
const Todo = ({children, onDelete}) => {
  ...
  <div className='todo-list-wrapper'>
    ...
    <button onClick={onDelete} className='todo-delete'>
      <svg className="bin-delete" fill="#FFFFFF" xmlns="http://www.w3.org/2000/svg"  
          viewBox="0 0 50 50">
